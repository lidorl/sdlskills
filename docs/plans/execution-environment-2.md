...
- [x] **Step 1: Add a "Build the repository catalog" step** after the repo-mapping / before the autonomy interview. Content:
...
- [x] **Step 2: Add a "Meta-root wiring" step**:
...
- [x] **Step 3: Update the skill's frontmatter `description`** to mention "builds the repository catalog".

- [x] **Step 4: Verify** — re-read `skills/setup-sdlc/SKILL.md`; confirm the catalog step precedes the autonomy interview and references `repos.yml` / `repos.example.yml` correctly.

- [x] **Step 5: Commit** — `git commit -m "feat(setup-sdlc): build the repository catalog"`
...
- [x] **Step 1: Insert a new "Step 2.5: Resolve the repository set"** between Step 2 (understand the current system) and Step 3 (technical brainstorm). Content:
...
- [x] **Step 2: Add `repos:` to the frontmatter template** in Step 4 (`repos: [key, key]`).

- [x] **Step 3: Add a new required section** to the Step 4 template, after `## API Surface Changes`:
...
- [x] **Step 4: Update Step 6 (approval)** to add: "Lock the `repos:` list — it is fixed at approval. Later phases read only this list." And add to the "on approval" actions: nothing extra (feat-req → DESIGNED already there).

- [x] **Step 5: Add a constraint**: "A repo discovered after approval → add it to `repos:` and `## Repositories in Scope` with a dated note, re-run `/classify-change` for it, get a lightweight re-approval. Not a full redo."

- [x] **Step 6: Verify** — re-read; confirm section order (`Context → Proposed Design → Data Model → API Surface → Repositories in Scope → UI/UX → Alternatives → Testing → Rollout → Security → Open Items`) and that `render-status.mjs`'s design-doc handling still works (it only reads frontmatter `status`).

- [x] **Step 7: Commit** — `git commit -m "feat(write-design-doc): repository selection and binding"`
...
- [x] **Step 1: Add a "Multi-repo" subsection**: run Steps 1–3 **once per repo** in the approved design doc's `repos:` list. Output one verdict per repo (`api: standard`, `web: trivial`). The **effort tier** is the highest across repos. `hotfix` is decided once for the whole effort (Step 1), never per repo.

- [x] **Step 2: Update the output format** in Step 4 to a per-repo table: `key │ tier │ rule │ justification`.

- [x] **Step 3: Update Step 4's routing** — "standard" (any repo) → the effort enters/continues the standard pipeline; a repo that is `trivial` still gets its plan items + tests + PR but no separate design work (it's already covered by the effort's one design doc).

- [x] **Step 4: Verify** — re-read; confirm the per-repo loop doesn't contradict the effort-level `hotfix` carve-out.

- [x] **Step 5: Commit** — `git commit -m "feat(classify-change): per-repo tier verdicts"`
...
- [x] **Step 1: Add to Step 2** (write the plan): the plan must include, near the top, a `## Repositories` section — per repo: `key`, role (`owner`|`consumer`), tier (from `classify-change`), one line on what changes. And at the bottom, a `## Pull Requests` section:
...
- [x] **Step 2: Add to Step 2**: file references in task `Files:` blocks use `<key>:<path>`.

- [x] **Step 3: Add to Step 3 (stamp frontmatter)**: also copy `repos:` from the design doc into the plan frontmatter.

- [x] **Step 4: Add a constraint**: "Merge order lives in the design doc's Rollout & Migration section under a `### Merge order` subheading (ordered list of repo keys + one-line reason). Do not duplicate it in the plan."

- [x] **Step 5: Verify** — re-read; confirm `## Repositories` and `## Pull Requests` are named exactly (parsed by `render-status.mjs` in Task 9).

- [x] **Step 6: Commit** — `git commit -m "feat(write-execution-plan): per-repo sections and PR tracking"`
...
- [x] **Step 3: In `CLAUDE.process.md`**, add a `## Working in the meta-root` section (see Task 8) — placeholder note here, full text in Task 8.

- [x] **Step 4: Verify** — re-read both skills; confirm the history template still leads with frontmatter + `#` title (so `render-status.mjs` title extraction is unaffected).

- [x] **Step 5: Commit** — `git commit -m "feat: multi-repo development wiring + per-repo history summary"`
...
- [x] **Step 1: Add a step** after a task is selected and before routing to its phase: "If the selection is `DESIGNED`/`PLANNED`/`IN_PROGRESS` and has a design doc with a `repos:` list, run `node scripts/assemble-env.mjs <slug>` to reconstruct `workspace/`. Report any mismatch warning to the user."

- [x] **Step 2: Add**: when reporting an `IN_PROGRESS` effort's state, read the plan's `## Pull Requests` table and summarise (`api merged, web open, shared-types not started`).

- [x] **Step 3: Verify** — re-read; confirm it doesn't assemble for `ACCEPTED` items (no design doc yet).

- [x] **Step 4: Commit** — `git commit -m "feat(select-next-task): assemble workspace on resume"`
...
- [x] **Step 1: Add a step** to the `close-out` checklist, before the status flip: "For each repo in the plan's `## Repositories`: run `gh pr view <pr-url> --json state,mergedAt` and confirm `MERGED`. Verify merge order matches the design doc's `### Merge order`. If any PR is not merged, stop — the effort is not done; report which repos are outstanding."

- [x] **Step 2: Add** as the skill's final printed line: `run: node scripts/clean-env.mjs` (manual workspace cleanup — not automatic).

- [x] **Step 3: In `CLAUDE.process.md` Definition of Done**, add: "- [ ] A merged PR in every repo listed in the plan's `## Repositories`; links in `## Pull Requests`; merge order per the design doc's `### Merge order` (verified live via `gh`)."

- [x] **Step 4: Verify** — re-read; confirm the PR check precedes the `status: IMPLEMENTED` flip.

- [x] **Step 5: Commit** — `git commit -m "feat(close-out): live cross-repo PR verification"`
...
- [x] **Step 1: Add `## Working in the meta-root`** (after the Lifecycle section):
...
- [x] **Step 2: Add `repos.yml` and `repos/<key>.md`** to the Artifacts table.

- [x] **Step 3: Add a note to the State table**: "`select-next-task` runs `assemble-env.mjs` before routing a `DESIGNED`+ effort."

- [x] **Step 4: Update the `## Setup` line** to mention the catalog: "…maps the repo, **builds the repository catalog**, records the autonomy policy…".

- [x] **Step 5: Verify** — re-read the whole file; confirm no section-title drift vs. the skills that reference it (Autonomy Policy, State, Definition of Done).

- [x] **Step 6: Commit** — `git commit -m "docs(process): meta-root model, catalog, DoD"`
...
- [x] **Step 1: Write the failing test** (`scripts/render-status.test.mjs`):
...
- [x] **Step 6: Commit** — `git commit -m "feat(render-status): cross-repo PR progress; fix template count"`
...
- [x] **Step 1: Add a short "Multi-repo by default" block** to `site/index.html` — one section explaining the meta-root + `workspace/` + catalog model, in the existing drafting-set visual system (no new tokens, no new fonts). Link the concept to the 2-repo example (BRAINSTORMING.md §3) as "walkthrough coming".

- [x] **Step 2: Update `README.md`** — the "How it works" / lifecycle prose gains a sentence on multi-repo; the mermaid diagram gets a note that Design resolves the repo set and Development assembles `workspace/`.

- [x] **Step 3: Verify** — open `site/index.html` in a browser (or screenshot) once; confirm the new block matches the page's type scale and both themes.

- [x] **Step 4: Commit** — `git commit -m "docs: multi-repo on the landing page and README"`
...
- [x] `gather-open-items` may need an explicit task for the `<key>:` path prefix in extracted items — verify during execution.
- [x] Minimal `docs/design/architecture.md` for the kit — decide during T8.
- [x] Existing single-repo adopter migration (`setup-sdlc --migrate`) — separate feature request.
